...
output "workload_identity_provider_resource_name" {
  ...
  value       = length(var.github_repositories) > 0 ? module.workload_identity[0].workload_identity_provider_resource_name : null
}

output "cicd_service_account_email" {
  description = "Email of the CI/CD service account for GitHub Actions"
  value       = length(var.github_repositories) > 0 ? module.workload_identity[0].service_account_email : null
}
